...
mSampleRate (0)
{
    ...
    mState->state = ValueTree ("TapeDelay");
}
...
void TapeDelayAudioProcessor::fillDelayBuffer (AudioSampleBuffer& buffer, const int channelIn, const int channelOut, const int64 writePos, float startGain, float endGain)
{
...
void TapeDelayAudioProcessor::feedbackDelayBuffer (AudioSampleBuffer& buffer, const int channelIn, const int channelOut, const int64 writePos, float startGain, float endGain)
{
    if (mDelayBuffer.getNumSamples() > writePos + buffer.getNumSamples()) {
